...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        placeTiles = new Stack<LetterTile>();
    ...
    public boolean onStartGame(View view) {
        ...
        {
            if(choice==0)
                setter+=word1.charAt(j++);
            else
                setter+=word2.charAt(k++);
        ...
        messageBox.setText(setter);
        for(i=setter.length()-1;i>=0;i--)
        {
            stackedLayout.push(new LetterTile(this, setter.charAt(i)));
        }
        return true;
    }
    ...
    public boolean onUndo(View view) {
        if(!placeTiles.empty())
        {
            placeTiles.pop().moveToViewGroup(stackedLayout);
        }
        return true;
    }
}
